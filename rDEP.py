...
class DEP(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        lossDil = cp.sum(cp.multiply(nuPos,cp.pos(xiPos)))/Mpos+cp.sum(cp.multiply(nuNeg,cp.pos(xiNeg)))/Mneg+self.C*cp.norm(w-ref,1)
        ...
        lossEro = cp.sum(cp.multiply(nuPos,cp.pos(etaPos)))/Mpos+cp.sum(cp.multiply(nuNeg,cp.pos(etaNeg)))/Mneg+self.C*cp.norm(m-ref,1)
